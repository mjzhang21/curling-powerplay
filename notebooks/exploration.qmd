...
df_ppl_fs = df_ppl[df_ppl['shot'] == 1]
...
```{python}
df_ppl_fs = df_ppl_fs[['id', 'game', 't1','t2','end','hammer_end',
...
df_ppl_fs = df_ppl_fs.rename(columns={
    'r1x': 'nh0x',
    'r1y': 'nh0y',
    'y1x': 'h0x',
    'y1y': 'h0y',
...
```
```{python}
df_ppl_fs['nhteam'] = np.where(df_ppl_fs['t1']!=df_ppl_fs['hteam'],
                               df_ppl_fs['t1'], df_ppl_fs['t2'])

df_ppl_fs['score_diff_before'] = np.where(
    df_ppl_fs['t1'] == df_ppl_fs['hteam'],
    df_ppl_fs['t1before'] - df_ppl_fs['t2before'],
    df_ppl_fs['t2before'] - df_ppl_fs['t1before']
)
```
...
numeric_features = ['score_diff_before','end','percent']
categorical_features = ['turn','throw',]

X_num_scaled = pd.DataFrame(
    StandardScaler().fit_transform(df_ppl_fs[numeric_features]),
    columns=numeric_features,
    index=df_ppl_fs.index
)

# One-hot encode categorical features
X_cat = pd.get_dummies(df_ppl_fs[categorical_features], drop_first=False)
...
plt.figure(figsize=(8,6))
plt.scatter(X_pca[:,0], X_pca[:,1], c=hdb_labels, cmap='tab10', alpha=0.7)
plt.xlabel('PC1')
plt.ylabel('PC2')
plt.title('HDBSCAN Clusters projected on PCA')
plt.colorbar(label='Cluster')
plt.show()
...
plt.figure(figsize=(8,6))
plt.scatter(X_tsne[:,0], X_tsne[:,1], c=hdb_labels, cmap='tab10', alpha=0.7)
plt.xlabel('t-SNE Component 1')
plt.ylabel('t-SNE Component 2')
