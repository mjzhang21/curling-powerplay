...
for cluster in unique_clusters:
    ...
    # nh0
    plt.scatter(df.loc[mask, 'nh0x'],
                df.loc[mask, 'nh0y'],
                alpha=0.7, s=60, marker='o', label='nh0')
    
    # nh1
    plt.scatter(df.loc[mask, 'nh1x'],
                df.loc[mask, 'nh1y'],
                alpha=0.7, s=60, marker='s', label='nh1')
    
    # h0
    plt.scatter(df.loc[mask, 'h0x'],
                df.loc[mask, 'h0y'],
                alpha=0.7, s=60, marker='^', label='h0')
    
    plt.title(f"HDBSCAN Cluster {cluster}")
    plt.legend()
    plt.show()
...
def summarize_clusters(df, group_var, variables, groups_to_use=None, alpha=0.05):
    if groups_to_use is None:
        groups_to_use = sorted(df[group_var].dropna().unique())
    ...
    for var in variables:
        ...
        for g in groups_to_use:
            data = df_var[df_var[group_var] == g][var]
            n = len(data)
            mean_val, lower, upper = bootstrap_mean_ci(data, alpha=alpha, n_boot=10000, random_state=42)
            descriptives[g] = f"{mean_val:.2f} ({lower:.2f}--{upper:.2f})"
        ...
        # Games–Howell post hoc if significant
        pairwise_str = ""
        if pval <= alpha:
            gh = pg.pairwise_gameshowell(dv=var, between=group_var, data=df_var)
            comparisons = []
            for _, row in gh.iterrows():
                comparisons.append(f"{row['A']} vs {row['B']} ($p={row['pval']:.3f}$)")
            pairwise_str = "; ".join(comparisons)
        
        summary_rows.append({
            "Variable": var,
            **{f"C{g}": descriptives[g] for g in groups_to_use},
            "p-value": f"{pval:.3f}",
...
```{python}
variables = ['score_diff_before','percent','h_during','nh_during']
group_var = 'hdbscan_cluster'
groups_to_use = [0,1,2,3]  # only the clusters you want

summary_df = summarize_clusters(df, group_var, variables, groups_to_use)
summary_df
```
...
latex_table = summary_df.to_latex(
    index=False,
    escape=False,  # keep $ and parentheses for CI
    column_format="l" + "c"*len(groups_to_use) + "cc"  # left + centers
)
