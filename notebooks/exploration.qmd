...
```
```{python}
df_ppl = df_shots[df_shots['powerplay'] == 0]
df_ppl_fs = df_ppl[df_ppl['shot'] == 0]
```

```{python}
plot_curling_sheet()
plt.scatter(df_ppl_fs['r1x'], df_ppl_fs['r1y'], alpha=0.5, label='R1', s=50)
plt.scatter(df_ppl_fs['y1x'], df_ppl_fs['y1y'], alpha=0.5, label='Y1', s=50)
plt.legend()
plt.show()
```
```{python}
plot_curling_sheet()
plt.scatter(df_ppl_fs['r2x'], df_ppl_fs['r2y'], alpha=0.5, label='R1', s=50)
plt.scatter(df_ppl_fs['y2x'], df_ppl_fs['y2y'], alpha=0.5, label='Y1', s=50)
plt.legend()
plt.show()
```
```{python}
plot_curling_sheet()
plt.scatter(df_ppl_fs['shooterx'], df_ppl_fs['shootery'],alpha=0.5, label='shooter',s=50)
plt.legend()
plt.show()
```
...
for pair in pairs:
    counts = df_ppl_fs[pair].value_counts(normalize=True) * 100  # multiply by 100 for %
