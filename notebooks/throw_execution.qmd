...
formula = (
    ...
    ' + C(s4_throwgrp, Treatment(reference="Draw"))'
    ' + C(s6_throwgrp, Treatment(reference="Draw"))'
    ' + C(end_grp, Treatment(reference="6"))*score_diff_before_c'
    ' + strength_diff_z'
)
...
needed_cols = [
    "h_success", "match_id", "end_grp", "score_diff_before_c", "strength_diff_z",
    "s2_percent25_c", "s4_percent25_c", "s6_percent25_c",
    "s2_throwgrp", "s4_throwgrp", "s6_throwgrp"
]
...
for train_idx, test_idx in gkf.split(df_model, df_model["h_success"], groups=df_model["match_id"]):
    train_df = df_model.iloc[train_idx].copy()
    test_df = df_model.iloc[test_idx].copy()

    for c in ["s2_throwgrp", "s4_throwgrp", "s6_throwgrp", "end_grp"]:
        train_df[c] = train_df[c].astype("category").cat.remove_unused_categories()
        test_df[c] = pd.Categorical(test_df[c], categories=train_df[c].cat.categories)

    test_df = test_df.dropna(subset=["s2_throwgrp", "s4_throwgrp", "s6_throwgrp", "end_grp"]).copy()

    model = smf.logit(formula=formula, data=train_df).fit(disp=0)
    p_test = model.predict(test_df)
    y_test = test_df["h_success"].values

    auc_scores.append(roc_auc_score(y_test, p_test))
    brier_scores.append(brier_score_loss(y_test, p_test))
    acc_scores.append(accuracy_score(y_test, (p_test >= 0.5).astype(int)))

    all_y.extend(y_test)
    all_p.extend(p_test)

print(f"Mean AUC: {np.mean(auc_scores):.3f} (SD {np.std(auc_scores):.3f})")
print(f"Mean Brier: {np.mean(brier_scores):.3f} (SD {np.std(brier_scores):.3f})")
print(f"Mean Accuracy: {np.mean(acc_scores):.3f} (SD {np.std(acc_scores):.3f})")
...
plt.title("Grouped 5-Fold CV ROC Curve")
plt.legend()
plt.show()
```
