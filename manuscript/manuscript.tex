...
determining whether the hammer team scores more than two points in a Power Play
...
precise shot-making. Even while shot 6 has a larger role, all hammer team shot
execution scores have a critical role in deciding the end outcome. This implies
...
Overall, the results suggest that successful Power Play use is primarily an
execution problem, with shot-call differences playing their largest role in
early and mid-end structure. Higher execution on hammer shots 2, 4, and 6 was
associated with increased odds of scoring $\ge 2$, with the largest association
for shot 6 execution. Shot-call associations were smaller and depended on shot
number. Relative to the default Draw call, deviations toward more defensive or
higher-variance options were associated with lower scoring outcomes, most
notably a Guard call on shot 2 and a Peel/Raise call on shot 4. Match context
also mattered: conversion odds were lower in early ends and in end 8, and
higher relative team strength was associated with more frequent multi-point
scoring. Future work that incorporates stone-coordinate measures of board
state, models interactions between shot type and match context, or uses the
full end score rather than a binary outcome can refine these associations into
more explicit tactical guidelines for Power Play calling and response.
